...
def p_losses(denoise_model, x_start, t, cond, feat, data, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, noise=None, loss_type="l1"):
    ...
    if loss_type == 'l1':
        ...
        # loss = F.l1_loss(predicted_noise_sim, predicted_noise)
    elif loss_type == 'l2':
        loss = F.mse_loss(noise, predicted_noise)
        # loss = F.mse_loss(predicted_noise_sim, predicted_noise)
    elif loss_type == "huber":
        loss = F.smooth_l1_loss(noise, predicted_noise)
        # loss = F.smooth_l1_loss(predicted_noise_sim, predicted_noise)
    else:
