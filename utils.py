...
def eval_autoencoder(test_loader_f, test_loader_cf, autoencoder, n_max_nodes, device):
    Gs = []
    Gs_rec = []
    sims = []
    for data_f in test_loader_f:
        Gs = []
        Gs_rec = []

        data_f = data_f.to(device)
        adj_rec = autoencoder(data_f)
        adj_rec[adj_rec>0.5] = 1
        adj_rec[adj_rec<=0.5] = 0

        for i in range(data_f.A.size(0)):
            Gs.append(construct_nx_from_adj(data_f.A[i,:,:].detach().cpu().numpy()))
    ...
    for data_cf in test_loader_cf:
        Gs = []
        Gs_rec = []

        data_cf = data_cf.to(device)
        adj_rec = autoencoder(data_cf)
        adj_rec[adj_rec>0.5] = 1
        adj_rec[adj_rec<=0.5] = 0

        for i in range(data_cf.A.size(0)):
            Gs.append(construct_nx_from_adj(data_cf.A[i,:,:].detach().cpu().numpy()))
            Gs_rec.append(construct_nx_from_adj(adj_rec[i,:,:].detach().cpu().numpy()))
