...
parser.add_argument('--epochs-autoencoder', type=int, default=6)
parser.add_argument('--hidden-dim-encoder', type=int, default=64)
# parser.add_argument('--hidden-dim-encoder', type=int, default=16)
...
# parser.add_argument('--n-max-nodes', type=int, default=48)
...
# parser.add_argument('--spectral-emb-dim', type=int, default=262)
parser.add_argument('--variational-autoencoder', action='store_true', default=True)
# parser.add_argument('--epochs-denoise', type=int, default=150)
...
# filename = f'processed_data/nba-0508.pt'

if os.path.isfile(filename):
    data_lst = torch.load(filename)
    # for i in range(len(data_lst)):
        # print(data_lst[i].edge_index.shape)
    ...
    print(f'No such {filename} exists. Please check again.')
    
print(f'data_lst: {len(data_lst)}')
...
np.savetxt('test_idx-cf.txt', np.array(test_idx))
...
if args.train_denoiser:
    ...
    for epoch in range(1, args.epochs_denoise+1):
        ...
        for data in train_loader:
            ...
            data.edge_index = data.T_cf_edge_index
            x_g_cf = autoencoder.encode(data)
            t = torch.randint(0, args.timesteps, (x_g.size(0),), device=device).long()
            loss, noise = p_losses(denoise_model, x_g, t, data.x, x_g_cf, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, loss_type="huber") 
            loss.backward()
        ...
        for data in val_loader:
            ...
            x_g = autoencoder.encode(data)
            t = torch.randint(0, args.timesteps, (x_g.size(0),), device=device).long()
            loss, noise = p_losses(denoise_model, x_g, t, data.x, data.T_cf_edge_index, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod, loss_type="huber")
...
for k, data in enumerate(tqdm(train_loader, desc='Processing train set',)):
    data = data.to(device)
    bs = 5
    samples = sample(denoise_model, data, data.x, data.T_cf_edge_index, latent_dim=args.latent_dim, timesteps=args.timesteps, betas=betas, batch_size=bs)
    x_sample = samples[-1]
...
torch.save(adjs, f'NBA-0117-cf-all.pt')
